# ─────────────────────────────────────────────────────────────────────────────
# examples/ecs-service-smoke/main.tf
# Smoke: ECS Cluster + Task + Service (con ALB HTTP) en la VPC por defecto
# ─────────────────────────────────────────────────────────────────────────────
...
# ── Descubre la VPC por defecto y sus subnets ────────────────────────────────
...
# Tomamos al menos 2 subnets de la VPC por defecto (para ALB multi-AZ)
locals {
...
module "ecs_cluster" {
  ...
  tags = {
    Purpose = "ecs-service-smoke"
  }
}

# ── IAM para Task/Execution Role (módulo existente) ──────────────────────────
module "ecs_iam" {
...
module "ecs_service" {
  ...
  # Usamos todas las subnets de la VPC por defecto para simplificar el smoke
  subnet_ids         = local.subnets_all
  security_group_ids = [aws_security_group.tasks.id]
  assign_public_ip   = true # en VPC por defecto, las subnets suelen ser públicas
  ...
  # Opcional: capacity providers y autoscaling (desactivado en smoke)
